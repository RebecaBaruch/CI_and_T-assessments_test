...
function calculaPorcentagemSorteio(assinante,minutosAssistidos) {
  let minFilter = [];//guardaos valores filtrados
  let totalMin = 0;//armazena o total de minutos assistidos
  let per = [];//guarda a porcentagem por pessoa
  
  for(i=0; i<assinante.length; i++){//enquanto for menor que 'assinante'...
    //verifica se tem assinatura. Se sim, dobra o valor de minutos assistidos com index correspondente
    (assinante[i]==true)?minFilter.push(minutosAssistidos[i]*2) : minFilter.push(minutosAssistidos[i]);
    //em seguida guarda tudo em minFilter
  }
  
  //soma o total de minutos assistidos
  ...
  for(k=0;k<minFilter.length;k++){
    ...
    per.push(numPer);// guarda o resultado
  }
